...
#' @param name String of the element DCN. The provided name will be converted to be IFB database compliant (no special characters, all lowercase, spaces replaced with underscores.)
#' @param label Label for the element.
#' @param description Text description of the element.
#' @param data_type Integer indicating data type of the element. List of iFormBuilder data types: [https://iformbuilder.zendesk.com/hc/en-us/articles/201702880-Data-Types-and-Related-Details](https://iformbuilder.zendesk.com/hc/en-us/articles/201702880-Data-Types-and-Related-Details)
#' @param data_size *Optional* - length of the element; defaults to 100.
#' @param optionlist_id *Optional* - id of the option list to assign if a Select, Picklist, or Multi Picklist element is created.
...
#'   }
#' @export
create_element = function(server_name, profile_id, access_token, page_id, name, label, description="", data_type, data_size=100, optionlist_id) {
  ...
  if (!missingArg(optionlist_id)) {
    page_attributes <- list(name=name,label=label,description=description,data_type=data_type,data_size=data_size,optionlist_id=optionlist_id)
  }
  else {
    page_attributes <- list(name=name,label=label,description=description,data_type=data_type,data_size=data_size)
  }
  page_attributes <- rjson::toJSON(page_attributes)
...
#' @param access_token Access token produced by \code{iformr::get_iform_access_token}
#' @param page_id Page ID where the new element will be created.
#' @param fields *Optional* - Defaults to 'all', which returns all fields for each element. Optionally, a character vector of fields to return can be provided.
#' @return Dataframe containing a row for each element and a column for each element field.
#' @examples
#' \dontrun{
#' # Get access_token
#' access_token <- get_iform_access_token(
...
#'   }
#' @export
retrieve_element_list <- function(server_name, profile_id, access_token, page_id, fields='all') {
  ...
  all_fields <- c('global_id', 'version', 'label', 'description', 'data_type',
                  ...
                  'is_required', 'condition_value', 'client_validation', 'is_disabled',
                  'reference_id_1', 'attachment_link', 'is_readonly', 'validation_message',
                  'smart_tbl_search', 'smart_tbl_search_col', 'is_encrypt', 'is_hide_typing',
                  'keyboard_type', 'dynamic_label', 'weighted_score', 'localizations')
  if (fields == 'all') {fields <- all_fields}
  else {
    fields <- sapply(fields, tolower) #field vector to lower
    fields <- intersect(fields, all_fields) #only include fields having a match in all fields
  }
  ...
  url <- paste0(api_v60_url(server_name = server_name),
                profile_id, "/pages/", page_id, "/elements?fields=", fields, "&limit=100&offset=0")
  bearer <- paste0("Bearer ", access_token)
  r <- httr::GET(url,
                 httr::add_headers('Authorization' = bearer),
                 encode = "json")
